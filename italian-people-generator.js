...
var data_mapping = {
    'personal-data': ['firstname', 'surname', 'age', 'bloodType', 'city', 'father', 'mother'],
    'app-data': ['id_num', 'email', 'registerDate', 'ticketNumber', 'username'],
    'professional-data': ['jobs', 'salary'],
    'miscelaneous': ['favFruits', 'movies', 'cat', 'dog']
}

function get_column_family(att_name) {
    for (var key in data_mapping) {
        var value = data_mapping[key];        
        if (value.indexOf(att_name) >= 0)
            return key;
    }
    throw 'Unexpected column name ' + att_name
}

function quote_and_comma(list) {
    return '\'' + list.join('\', \'') + '\''
}

function split_model_as_columns(entity, model, id) {
...
var table_name = 'italians'
console.log('create', 
    quote_and_comma([table_name].concat(Object.keys(data_mapping)))
)

for (let i = 0; i < 10000; ++i) {
    let person = randPerson(i+100);

    split_model_as_columns(table_name, person, i+1)
